...
ALTER TABLE case_ticket ADD COLUMN action_date DATE;
...
ALTER TABLE case_ticket ADD COLUMN install_date DATE;

COMMENT ON COLUMN case_ticket.ticket_type IS 'INSTALLATION | CM. Which board family the row came from.';
COMMENT ON COLUMN case_ticket.action_date IS 'Board "RE Action" date. SLA = action_date - open_date <= sla_days.';
COMMENT ON COLUMN case_ticket.install_date IS 'Installation tickets: later end of the TimeLine column.';

CREATE INDEX idx_case_ticket_type_install ON case_ticket (ticket_type, install_date);
CREATE INDEX idx_case_ticket_type_open    ON case_ticket (ticket_type, open_date);
...
-- -----------------------------------------------------------------------------
ALTER TABLE case_ticket_sync_run ALTER COLUMN service_line DROP NOT NULL;
ALTER TABLE case_ticket_sync_run ADD COLUMN ticket_type VARCHAR(16) NOT NULL DEFAULT 'CM';

COMMENT ON COLUMN case_ticket_sync_run.service_line IS
